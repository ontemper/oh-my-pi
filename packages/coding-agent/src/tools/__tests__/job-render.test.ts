...
import { type JobToolDetails, jobToolRenderer } from "../job";
...
function renderJobText(details: JobToolDetails, expanded = false): string {
	const component = jobToolRenderer.renderResult(
		{ content: [{ type: "text", text: "Listed background jobs" }], details },
...
describe("jobToolRenderer task model badges", () => {
	beforeAll(async () => {
